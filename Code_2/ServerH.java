...
public class ServerH extends Thread{
	...
	public ServerH(){ 
		...
		ranks.put("hum", 80); ranks.put("no2", 70); ranks.put("o3", 30);
		units.put("hum", "%"); units.put("no2", "ug/m3"); units.put("o3", "ug/m3");
	}
	
	public static String getValues() {
		String reply = ">>> ServerH Data <<<\n";
			...
			while (running) {
				try {
					...
					if (received.equals("Model")) {
						buf = new String("ServerH").getBytes();
					}
					else if (received.equals("End")) {
						running = false;
						buf = new String("ServerH shutdown succesfully.").getBytes();
					}
						...
						if (frequency != Integer.parseInt(parts[1])*1000) {
							...
							buf = new String("Frequency in ServerH changed succesfully.").getBytes();
